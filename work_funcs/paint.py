...
from work_funcs.draw2 import draw
...
generalPic = Image.open("assets/blhx_raw/general.jpg").convert("RGB")
leftPath = os.path.realpath(os.path.join(CWD, "../assets/blhx_raw/left"))
rightPath = os.path.realpath(os.path.join(CWD, "../assets/blhx_raw/right"))
...
u = Image.open(os.path.realpath(os.path.join(CWD, "../assets/blhx_raw/up.jpg"))).convert("RGB")
d = Image.open(os.path.realpath(os.path.join(CWD, "../assets/blhx_raw/down.jpg"))).convert("RGB")
...
def drawN(img_path, w=1, h=1, blur=0):
    """
    生成碧蓝航线 绘画板像素画
    ...
    paint = draw(img_path, 37 * w, 22 * h, blur)
    # 从BGR到RGB
    paint = cv2.cvtColor(paint, cv2.COLOR_BGR2RGB)
    # 转换为 PIL.Image
    paint_pil = Image.fromarray(paint.astype('uint8')).convert('RGB')
